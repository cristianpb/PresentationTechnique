...
if __name__ == '__main__':
     ...
     #img = cv2.imread('Fish/aug_120_8601.jpeg', 0)
     img = cv2.imread('Fish/aug_120_8601.jpeg', cv2.IMREAD_COLOR)
     b, g, r = cv2.split(img)
     img = cv2.merge([r, g, b])
     desked_img = deskew(img)
 
     f, (ax1, ax2) = plt.subplots(1, 2)
     ax1.imshow(img)
     ax1.set_xticks([])
     ax1.set_yticks([])
     ax2.imshow(desked_img)
     ax2.set_xticks([])
     ax2.set_yticks([])
     plt.savefig('deskewed.png')
 
     hog = get_hog();
     hog_descriptors = []
     hog_descriptors.append(hog.compute(desked_img))
